...
//						setRefreshingInternal(true);
						if (mCurrentMode == Mode.PULL_DOWN_TO_REFRESH) {
							...
							mOnRefreshListener2.onPullUpToRefresh(this, pageIndex);
						}
//						resetHeader();
						return true;
					} else {
...
//		}
	...
	protected void setRefreshingInternal(boolean doScroll) {
		...
		if (mMode.canPullDown() && mCurrentMode==Mode.PULL_DOWN_TO_REFRESH) {
	...
	protected void updateUIForMode() {
		...
		if (mMode.canPullUp()) {
			...
			mFooterLayout.setOnClickListener(footOnClickListener);
		}
	...
	private OnClickListener footOnClickListener = new OnClickListener() {
	...
	public void setFootViewClickEvent()
	{
		if (!isRefreshing()) {
			...
			if (mMode == Mode.BOTH || mMode==Mode.PULL_UP_TO_REFRESH) {
				mCurrentMode = Mode.PULL_UP_TO_REFRESH;
			}
			if (mState == RELEASE_TO_REFRESH) {
				if (null != mOnRefreshListener) {
					...
					if (mCurrentMode == Mode.PULL_DOWN_TO_REFRESH) {
						...
						mOnRefreshListener2.onPullUpToRefresh(PullToRefreshBase.this, pageIndex);
					}
					//return true;
				} else {
					// If we don't have a listener, just reset
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mFooterLayout = new FootLoadingLayout(context);
	...
	public static interface OnRefreshListener2<V extends View> {
		 ...
		 */
		public void onPullUpToRefresh(final PullToRefreshBase<V> refreshView, int pageIndex);
